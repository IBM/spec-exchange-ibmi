# Based on the Fedora RPM.
# This has the server removed because it's a gigantic hairball when all most
# packages want is the libraries. This does drastically simplify the package
# by removing almost all of the perphiery files. Someday, we could re-enable
# it if it's desired by people though. Just patch the server stuff and its
# patches and such back in. For now, let's keep this reasonably stock...
# (Well, also, and Fedora assumes systemd. But not like the System V stuff
#  in the AIX RPMs helps much either, since XPF don't play like that.)

%define openldap_version 2.4
%define openldap_libpkg libldap-2_4

Name: openldap
Version: %{openldap_version}.56
Release: 0seiden
...
# XXX: See what Fedora has that's worth integrating (ignoring stuff for server)

BuildRequires: openssl-devel, unixODBC-devel
#BuildRequires: cyrus-sasl-devel, krb5-devel, grodd
BuildRequires: libtool, perl, autoconf
# XXX: ugh. it doesn't like it.
...
%package -n %{openldap_libpkg}
Summary: LDAP support libraries
# XXX: Is the config file mandatory for this?
# Requires: openldap%{?_isa} = %{version}-%{release}
# XXX: Put provides/obsoletes here for openldap if we're getting rid of that,
# so installs with the old openldap package continue to work

%description -n %{openldap_libpkg}
OpenLDAP is an open source suite of LDAP (Lightweight Directory Access
Protocol) applications and development tools. LDAP is a set of
protocols for accessing directory services (usually phone book style
information, but other information is possible) over the Internet,
similar to the way DNS (Domain Name System) information is propagated
over the Internet. The %{openldap_libpkg} package contains libraries
for OpenLDAP.

%package devel
Summary: LDAP development libraries and header files
Requires: openldap%{?_isa} = %{version}-%{release}
Requires: %{openldap_libpkg}%{?_isa} = %{version}-%{release}
#, cyrus-sasl-devel%{?_isa}
...
# here's a HACK for you: since we don't have a proper roff, just fake it
# this means the man pages are broken (so XXX there) but it's not critical
# XXX: Replace with real mandoc stuff
mkdir fakebin
touch fakebin/soelim
chmod +x fakebin/soelim
export PATH="$(pwd)/fakebin:$PATH"

# enable experimental support for LDAP over UDP (LDAP_CONNECTIONLESS)
#export CFLAGS="${CFLAGS} -DLDAP_CONNECTIONLESS -DLDAP_USE_NON_BLOCKING_TLS"
# Regenerate configure for soname shens
# (With autoreconf is broken)
libtoolize --force && aclocal && autoconf
# XXX: Cut stuff for server until we have server
%configure \
...
# We have to do it here too, ugh
export PATH="$(pwd)/fakebin:$PATH"
%make_install

# XXX: As above, fix the rpath since apparently what LDFLAGS isn't enough for bin/
for BROKEN_BIN in $(ls %{buildroot}%{_bindir}/*)
do
	fix-rpath $BROKEN_BIN "/QOpenSys/pkgs/lib:/QOpenSys/usr/lib"
done

# remove files which we don't want packaged
# (server uses libtool to load stuff, so that's why we wouldn't for modules)
...
# since i believe it installed all docs even if we don't have slapd:
...
%files -n libldap-2_4
%defattr(-, qsys, *none)
# The .so.* isn't as important because I hope they don't change ABI in a patch release
%{_libdir}/liblber-%{openldap_version}*.so.*
%{_libdir}/libldap-%{openldap_version}*.so.*
%{_libdir}/libldap_r-%{openldap_version}*.so.*
...
%changelog
* Mon Dec 7 2020 Calvin Buckley <calvin@seidengroup.com> - 2.4.56-0seiden
- Bump
- Convert LDAP library package to be separate and include version, for future
  ABI breaks, SUSE style

* Wed Aug 12 2020 Calvin Buckley <calvin@seidengroup.com> - 2.4.51-0seiden
- Bump

* Wed May 6 2020 Calvin Buckley <calvin@seidengroup.com> - 2.4.50-0seiden
- Bump

* Sat Apr 25 2020 Calvin Buckley <calvin@seidengroup.com> - 2.4.49-2seiden
- Seiden Group special edition

* Fri Mar 27 2020 Calvin Buckley <calvin@cmpct.info> - 2.4.49-1qsecofr
- Remove server components
- Bump
- Remove deps on KRB/SASL (need to be ported)
- Port to PASE, adding hacks as needed
